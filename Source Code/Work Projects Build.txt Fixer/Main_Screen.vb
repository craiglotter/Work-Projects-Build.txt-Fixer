...
Public Class Main_Screen
    ...
    Private Sub PreCount_Function(ByVal worker As BackgroundWorker)
        Try
            ...
            Dim backupfolder As String = (Application.StartupPath & "\").Replace("\\", "\") & "WPBF Backup " & Format(Now, "yyyyMMddHHmmss")
            ...
            If dinfo.Exists Then
                For Each subdir As DirectoryInfo In dinfo.GetDirectories
                    ...
                    If worker.CancellationPending Then
                        ...
                        Return False
                    End If
                    If My.Computer.FileSystem.FileExists((subdir.FullName & "\Build.txt").Replace("\\", "\")) Then
                        Dim buildstring As String = ""
                        Dim originalbuildstring As String = ""
                        Dim reader As StreamReader = My.Computer.FileSystem.OpenTextFileReader((subdir.FullName & "\Build.txt").Replace("\\", "\"))
                        If reader.Peek <> -1 Then
                            buildstring = reader.ReadLine()
                            originalbuildstring = buildstring
                            If buildstring.Length < 10 Or buildstring.Length > 11 Then
                                AlertMessage = AlertMessage & "Invalid Build.txt File: " & (subdir.FullName & "\Build.txt").Replace("\\", "\") & vbCrLf
                            Else
                                If buildstring.Length = 10 Then
                                    buildstring = buildstring.Replace(".", ".0")
                                End If

                            End If
                        Else
                            AlertMessage = AlertMessage & "Blank Build.txt File: " & (subdir.FullName & "\Build.txt").Replace("\\", "\") & vbCrLf
                        End If
                        reader.Close()
                        If Not buildstring = originalbuildstring Then
                            Try
                                My.Computer.FileSystem.DeleteFile((subdir.FullName & "\Build.txt").Replace("\\", "\"), FileIO.UIOption.OnlyErrorDialogs, FileIO.RecycleOption.DeletePermanently)
                                Dim writer As StreamWriter = My.Computer.FileSystem.OpenTextFileWriter((subdir.FullName & "\Build.txt").Replace("\\", "\"), False)
                                writer.WriteLine(buildstring)
                                writer.Close()
                                writer = Nothing
                            Catch ex As Exception
                                Error_Handler(ex, "Create Updated Build File")
                            End Try
                        End If
                    End If
                    inputlines = inputlines + 1
